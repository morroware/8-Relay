...
def setup_gpio():
    ...
    global button_handler, audio_player, audio_button1_handler, audio_button2_handler, initialized_pins
...
def trigger_relay(relay_num):
    """Trigger a relay for its configured duration with error handling"""
    ...
    # Enforce max concurrent triggers
    ...
    try:
        ...
        with stats_lock:
            stats['errors'] += 1
        # Try to ensure relay is off in case of error
        try:
            pin = config.RELAY_PINS[relay_num]
            off_state = GPIO.HIGH if config.RELAY_ACTIVE_LOW else GPIO.LOW
            GPIO.output(pin, off_state)
...
def cleanup_gpio():
    ...
    global cleanup_done, button_handler, audio_player, audio_button1_handler, audio_button2_handler
